...
E' interessante indagare quali sono le fasce orarie di maggior utilizzo della Intranet, per capire le abitudini degli utenti.\
Questo aspetto può essere significativo ad esempio per pianificare attività manutentive sul portale in fasce orarie poco "affollate".
...
Alla fascia quindi va aggiunto un opportuno _off-set_ in funzione dell'ora solare (+1) o legale (+2).
```{r}
richieste = richieste %>%
...
Si conferma la netta prevalenza dell'uso del browser Google Chrome.\
Interessante osservare che presso la sede di PN è presente una quota di richieste provenienti da browser Microsoft Edge (in particolare la nuova versione Chrome - based).
\
...
```
\
Da questa visualizzazione si nota alcune applicazioni (OdsCom, UserProfile, News) sono utilizzate presso le due sedi in misura proporzionale al numero di utenti potenziali (UD > PN).
\
...
```
\
Si può notare che il tempo di esecuzione presenta una distribuzione caratterizzata da una coda lunga a destra, con una tempo di esecuzione mediano di 0.185s
...
```{r}
ggplot(richieste)+
  geom_point(aes(x=exec_time, y=application))
...
Da notare che sia l'applicazione "Documenti" sia l'applicazione "OdSCom" sono aratterizzate dall'accesso al file system, partciolare che avrebbe potuto influire allo stesso modo sul tempo di esecuzione, cosa che invece non viene evidenziata dall'analisi.\
...
\
Innanzi tutto creo un nuovo campo logged_in impostandolo a TRUE o FALSE
...
\
Si passa quindi a plottare un boxplot sul tempo di esecuzione, suddiviso per richieste loggate e non loggate
...
A questo punto il fattore che influisce sul tempo di esecuzione è individuabile sulle parti di script che vengono eseguite solo ad utente loggato, come la generazione del menù personalizzato e la generazione dell'area delle notifiche.
...
Allo scopo si esamina la distribuzione del tempo di esecuzione nella fascia con maggior numero di richieste (8-9) con la fascia di fine giornata lavorativa (17-18):
\
...
```
\
Non si riscontrano differenze significative, pertanto si conclude che nelle fasce di punta non ci sia un degrado di prestazioni dei server interessati.


**(SUB) Quali sono le parole più ricercate in rubrica?**\

Con alcune semplici tecniche di text-mining si isolano le chiavi di ricerca 
