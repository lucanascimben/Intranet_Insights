...
```
\
Il grafico a barre evidenzia che dalla sede di Udine vengono generate ancora relativamente poche richieste rispetto al numero degli utenti della sede. Questo indica che la sede di Udine è ancora in una fase iniziale di adozione dello strumento, con notevole potenziale di crescita.\
\
...
Si raggruppano quindi le richieste per sede e per mese, visualizzando quindi il dato in un boxplot
...
```
Il boxplot evidenzia che l'andamento mensile delle richieste di Udine non ha avuto particolari picchi positivi o negativi. Le richieste dalla sede di Pordenone presentano invece una variabiltà maggiore.\
Questa maggior variabilità è probabilmente da collegare al numero più elevato di richieste.
\
\
...
E' verosimile che con il progredire del porting sulla intranet delle applicazioni Lotus e FileMakert, il trend per la sede di Udine migliorerà notevolmente.
\
\
...
L'IT dell'ente suggerisce l'utilizzo di Google Chrome per una migliore compatabilità con la suite Google Workspace adottata, ma per ragioni di compatibilità è normalmente ammesso l'uso di qualsiasi browser purchè supportato dal relativo produttore.\
\
...
Si conferma la netta predominanza di Goole Chrome.\
Interessante osservare che presso la sede di PN c'è una quota di richieste provenienti da browser Microsoft Edge in versione Chrome - based.
\
...
\
Si decide pertanto di verificare l'evoluzione nel tempo dell'uso di browser IE10/IE11
\
\
```{r message=FALSE, warning=FALSE}
...
```
\
L'analisi è di conforto, perchè è evidenyte il progressivo abbandono del browser IE10/IE11.\
I dati rilevati a dicembre 2022 mostrano infatti un numero residuo di richieste inferiore alle 100 unità per entrambe le sedi.\
\
